...
namespace Pseez.UI.IT.Areas.DataCenter.Controllers
{
    public class ServerInformationController : Controller
    {
        ...
        public ServerInformationController(IUnitOfWorkPseezEnt uow, IServerService serverService)
        {
        ...
        //
        // GET: /DataCenter/ServerInformation/
        public ActionResult Index()
        {
            //PseezErpContext PseezErpdb = new PseezErpContext();
            //return View(PseezErpdb.Servers);
            var a = _serverService.GetAll().MapModelToViewModel();
            return View(a);
        }

        public ActionResult ServerInformation(string ServerIP, string Username, string Password)
        {
            var stringWin32class = "Win32_OperatingSystem";
            var ServerInfo = ServerInfoWin32class(ServerIP, Username, Password, stringWin32class);
            ViewBag.IP = ServerIP;
            return PartialView("_ServerInformation", ServerInfo);
        }

        private List<string[]> ServerInfoWin32class(string strIP, string strUsername, string strPassword,
            string stringWin32class)
        {
            var ServerInfoResult = new List<string[]>();
            ...
            try
            {
                var ManagementScope1 = new ManagementScope(string.Format("\\\\{0}\\root\\cimv2", strIP), options);
                ManagementScope1.Connect();

                var objectGetOptions = new ObjectGetOptions();
                var managementPath1 = new ManagementPath(stringWin32class);
                var ManagementClass1 = new ManagementClass(ManagementScope1, managementPath1, objectGetOptions);

                var PropertyDataCollection1 = ManagementClass1.Properties;

                foreach (ManagementObject ManagementObject1 in ManagementClass1.GetInstances())
                {
                    foreach (var property in PropertyDataCollection1)
                    {
                        ...
                        try
                        {
                            ...
                            string[] stringWin32classPropertyValue =
                            {
                                stringWin32class, property.Name,
                                ManagementObject1.Properties[property.Name].Value.ToString()
                            };
                            ServerInfoResult.Add(stringWin32classPropertyValue);
                        }
                        catch
                        {
                        }
            ...
            {
                ServerInfoResult.Add(new[] {"اتصال به سرور امکان پذیر نشد", strIP, ex.Message});
            }
